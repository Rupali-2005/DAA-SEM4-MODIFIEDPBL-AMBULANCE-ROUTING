...
public class MainGUI extends JFrame {
    ...
    public MainGUI() {
        ...
        setVisible(true);
        refreshStats();
        log("AmbulanceOS ready. Graph: " + sys.getGraph().getNumNodes()
                + " nodes · " + sys.getGraph().getEdgeCount() + " edges.");
    }
    ...
    private JPanel buildHospitalTab() {
        ...
        JTextField specsF= field();  specsF.setToolTipText("e.g.  Cardiology, Trauma");
        JTextField bedsF =field();
        JTextField icuF =field();
        JTextField nodeF = field();
    ...
    private JPanel buildDispatchTab() {
        ...
        dispBtn.addActionListener(e -> {
            ...
            try {
                ...
                String result = sys.addPatientAndDispatch(p);
                log(result);
        ...
        reBtn.addActionListener(e -> {
            try { log(sys.dispatchForExisting(Integer.parseInt(pidF.getText().trim())));
        ...
        discBtn.addActionListener(e -> {
            try { log(sys.dischargePatient(Integer.parseInt(pidF.getText().trim())));
                  refreshPatientTable(); refreshStats(); pidF.setText("");
            } catch (NumberFormatException ex) { serr.setText("Enter a valid ID."); }
        });
    ...
    private JPanel buildTrafficTab() {
        ...
        rowC(formCard, gc, r++, "Traffic Level  (0=clear · 10=blocked)",
             inlinePanel(slider, valLbl));
        GridBagConstraints egc = clone(gc, 0, r++); egc.gridwidth = 2;
        formCard.add(errLbl, egc);
        ...
        simBtn.addActionListener(e -> {
            log(sys.simulateTraffic()); graphPanel.refresh(); status("Simulation complete.", TEXT_SEC);
        });
    ...
    private JPanel buildGraphSetupTab() {
        ...
        JLabel info = new JLabel(
            "Reset clears all edges. Load Demo restores<br>the default 10-node road network.</span></html>");
        GridBagConstraints ib = clone(gc, 0, 4); ib.gridwidth = 2;
        resetCard.add(info, ib);
        JPanel edgeCard = card("ADD EDGE");
        edgeCard.setLayout(new GridBagLayout());
        GridBagConstraints eg = baseGbc();
        JTextField fromF   = field();
        JTextField toF     = field();
        JTextField weightF = field();
        ...
        addEdgeBtn.addActionListener(e -> {
            ...
            try {
                ...
                if (result.startsWith(" ")) {
                    ...
                    edgeErr.setText(result.replace("X ", ""));
                }
    ...
    private JPanel buildGraphViewTab() {
        ...
        JLabel hint = new JLabel("  Amber = last dispatched route  ·  Line colour = traffic  ·  Dashed = blocked");
    ...
    private void refreshHospitalTable() {
        ...
        for (Hospital h : sys.getHospitals())
            hospitalModel.addRow(new Object[]{
                h.getId(), h.getName(),
                String.join(", ", h.getSpecializations()),
